 ...
 *
 */
class CMD_Shortcode_Gallery {

	static protected $instance;

	public static function get_instance() {
		if ( !self::$instance ) {
			self::$instance = new CMD_Shortcode_Gallery();
		}
	...
	/**
	 * Add class, title and data-img-description attrs for a tag.
	 *
	 * @param  string  $html
	 * @param  integer $id
	 * @param  string  $size
	...
	public function wp_get_attachment_link_filter( $html, $id = 0, $size = 'thumbnail', $permalink = false, $icon = false, $text = false ) {
		if ( $permalink ) {
			...
			$title = esc_attr( trim( strip_tags( $attachment->post_title ) ) );
			$desc = esc_html( $attachment->post_content );
			$html = str_replace( '<a ', "<a class='rollover rollover-zoom mfp-item mfp-image' title='{$title}' data-img-description='{$desc}' ", $html );
		}
		return $html;
	}
	...
	public function gallery_style_filter( $gallery_style ) {
		return str_replace( "class='", "class='cmd-gallery-container ", $gallery_style );
	}
	...
	protected function slideshow( $attachments, $atts, $instance, $id ) {
		...
		foreach ( $attachments as $id => $attachment ) {
			...
			$image_video_url = esc_url( get_post_meta( $id, 'video-url', true ) );
			if ( $image_video_url ) {
            ...
            $output .= wp_get_attachment_image( $id, 'full', false, $attr );
...
// create shortcode
CMD_Shortcode_Gallery::get_instance();